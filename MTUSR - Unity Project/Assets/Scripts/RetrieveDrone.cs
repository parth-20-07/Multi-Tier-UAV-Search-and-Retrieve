...
public class RetrieveDrone : MonoBehaviour
{
    #region Variables

    // The Global Drone Planner uses A-Star for path planning
    private Vector3 _goalPosition, _startPosition;
    [SerializeField] private GameObject droneObject, goalObject;
    ...
    private bool _goalPositionFound, _endGoalFound, _pathFound;
    ...
    class AStarNode
    {
        ...
        public void SelectNodeAsPath(Material selectionMaterial)
        {
            ...
            if (edge != null)
            {
                ...
                edge.material = selectionMaterial;
    ...
    void Start()
    {
        ...
        _startPosition = droneObject.transform.position;
        _startPosition.y += 100.0f;
        AStarNode startNode = new AStarNode(_startPosition, null, this.transform,_mappingLayer,nodeMaterial);
        int gCost, hCost;
        (gCost, hCost) = FindCost(_startPosition, _startPosition, _goalPosition);
    ...
    void Update()
    {
        if (!_endGoalFound && environment.environmentInstantiated && _goalPositionFound)
        {
            if (_openList.Count != 0)
            {
                ...
                foreach (var nodeInOpenList in _openList)
                {
                    if (neighborNodes.Contains(nodeInOpenList.GetTransform().position))
                    {
                        ...
                        (tempGCost, tempHCost) = FindCost(openNode.GetTransform().position,
                            nodeInOpenList.GetTransform().position, _goalPosition);
                ...
                foreach (var neighborPosition in neighborNodes)
                {
                    ...
                    int gCost, hCost;
                    (gCost, hCost) = FindCost(openNode.GetTransform().position, neighborPosition, _goalPosition);
                    gCost += openNode.GCost;
                    node.SetCost(gCost, hCost);
                    _openList.Add(node);
                    _nodeList.Add(node);
                    if ((neighborPosition - _goalPosition).sqrMagnitude < 5000.0f)
                    {
                            _endGoalFound = true;
                            break;
                    }
    ...
    private IEnumerator TravelOnPath()
    {
        ...
        while (true)
        {
            ...
            if (time >= 1.0f)
            {
                ...
                {
                    if (!goalObjectReached)
                    {
                        goalObject.transform.parent = droneObject.transform;
                        goalObjectReached = true;
                        i = 0;
                        _travelPath.Reverse();
                    }
                    else
    ...
    public void RetrieveGoalObject(Vector3 positionOfGoalObject)
    {
        _goalPosition = positionOfGoalObject;
        _goalPositionFound = true;
    }
